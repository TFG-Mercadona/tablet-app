...
export default function PlanogramaView() {
  ...
  const { url, familia, tiendaId } = useLocalSearchParams<{ url?: string; familia?: string; tiendaId?: string }>();
      ...
      <View style={styles.viewerCard}>
        {Platform.OS === 'web' ? (
          <Text style={{ color: UI.sub }}>
            En web abrimos el PDF en una pestaña nueva. Si no se abrió, vuelve atrás y pulsa de nuevo.
          </Text>
        ) : WebView ? (
          <WebView
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: UI.bg, padding: 20, alignItems: 'center' },
