...
const MOVE_ENDPOINT = (id: number) => `${API_BASE_URL}/api/tornillos/${id}/posicion`;
...
export default function OrganizarFamilia() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/tornillos/tienda/${id}/familia/${familia}/modulos`);
  ...
  const onCellPress = (fila: number, columna: number) => {
    ...
    const sameCell = selected.fila === fila && selected.columna === columna && selected.nombreModulo === moduloActual;
    if (sameCell) { setSelected(null); return; }
  ...
  const renderRow = (fila: number) => {
    const cols = items.filter(t => t.fila === fila).sort((a, b) => a.columna - b.columna);
    const maxCol = cols.length ? Math.max(...cols.map(t => t.columna)) : 0;
    const colIndices = maxCol ? Array.from({ length: maxCol }, (_, i) => i + 1) : [];
    ...
    return (
      <View key={`row-${fila}`} style={styles.row}>
        {colIndices.map(col => {
          ...
          return (
            <TouchableOpacity key={key} style={{ flex: 1 }} onPress={() => onCellPress(fila, col)} activeOpacity={0.85}>
              <View style={[
                styles.cell,
        ...
        <View style={styles.card}>
          ...
          <View style={styles.moduleHeader}>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              {selected && (
                <Text style={styles.moduleSub}>Seleccionado: {selected.productoCodigo} {selected.nombre} · toca otro tornillo para mover</Text>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  appBar: {
    ...
    shadowColor: '#000', shadowOpacity: Platform.OS === 'web' ? 0.06 : 0.12,
  ...
  card: {
    ...
    shadowColor: '#000', shadowOpacity: Platform.OS === 'web' ? 0.05 : 0.1,
  ...
  image: { width: CELL_IMG, height: CELL_IMG, resizeMode: 'contain', marginRight: 12 },

  infoCol: { height: CELL_IMG, justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 1 },
